...
def analyser_modules(modules):
    ...
    if len(modules) == 0:
        return stats
            ...
            temps = modules[i]["temps"]

    if temps != 0:
        ratio = modules[i]["cout"] / temps
    else:
        ratio = 
        for i in range(len(modules))-1:
            ratio = module[i]["cout"] / module[i]["temps"]
        if module[i]["cout"] / module[i]["temps_"] < module[i+1]["cout"] / module[i+1]["temps"]

    cout_moyen = for i in range(len(module))
